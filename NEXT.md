...
| **VOC** | 🔴 **내일 — 외국어 후보 추천 세부 언어 미반영** (운영팀 VOC 2026-06-02, 박민주 일본어 신청서 `1211dc1b`). 후보 추천(`db.py:641 list_candidate_teachers`)이 **과목 대분류(외국어=5)** 로만 거르고 신청서 세부 언어 태그(`recommendation_tag.tag_id`=51 일본어/52 중국어)를 무시 → 영어 선생님이 일본어 신청서에 추천됨. 선생님 측도 활동태그(`teacher_tags`) 대신 자기소개 컬럼(`teacher_introduction_activity_tag_foreign_language`) 비어있는지만 체크. LLM 입력 subject도 "외국어"로만 전달. **임팩트**: 외국어 풀 62,078명 중 일본어 가능 1,715명(2.7%)뿐. **자동 디스패치(auto_dispatch.py:192)도 동일 버그**(자동이라 더 위험). 수정: ①후보 SQL에 `recommendation_tag` ∩ `teacher_tags` tag_id 일치 조건 추가 ②LLM 입력에 세부 언어명 전달 ③auto_dispatch 점검. 정책 결정: 적용범위(외국어만 vs 전과목)·매칭강도(태그 1개 이상 vs 전부 보유) | 중간 | 큼(오추천) | 🔴 내일 |
| **A** | **"부모님께 전달" 버튼 구현** (현재 `index.html:2045` alert placeholder). 상세 패널에서 신청서를 `선생님 추천(SUGGESTED_TO_PARENT, 20)` 상태로 전환 + 부모님께 [선생님 추천] 알림톡 발송. 콘솔 API `POST /console/v1/admin/recommendations/status` 사용 (`send_alimtalk: true`로 알림톡 동시 발송). 작업 3곳: ①`console_client.py` `change_status(sid, status, send_alimtalk)` 추가 ②백엔드 신규 라우트 `POST /api/applications/{sid}/suggest-to-parent` ③프론트 버튼 fetch + 거절 응답(`DeniedRecommendationUpdate`) 처리. 전제조건: 현재 상태 접수안내(10) + 수락 선생님 1명 이상(미충족 시 콘솔이 거절). 정책 결정 필요: `send_alimtalk` 기본 true 고정 vs 운영자 토글 | 중간 | 큼 | 운영팀 요청 — 우선 검토 |
